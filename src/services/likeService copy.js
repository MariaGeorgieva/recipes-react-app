...
export async function getAllRecipeLikes(recipeId) {
    ...
    const result = await request.get(`${baseUrl}?where=${searchQuery}`);//TODO works 
    console.log("getAllRecipeLikes recipeId: ", recipeId, result);
    return result;
}

export async function getLikeByRecipeAndUser(likes, userId) {

    // const likes = await request.get(`${baseUrl}?distinct=${recipeId}&count`);//TODO works ;

    // console.log('getLikeByRecipeAndUser', likes);
...
export const likeRecipe = async (recipeId, userId) => {
    try {
        ...
        // Check if the recipe owner is liking his/her own recipe
